...
for filename in ['siouxfalls']:
    ...
    fig, ax = tapnx.plot_edges(G, pos=offset_outgoing_pos, edge_list=edge_list_outgoing, width=1.5, arrowsize=25, ax=ax)
    fig, ax = tapnx.plot_edges(G, pos=offset_incoming_pos, edge_list=edge_list_incoming, width=1.5, arrowsize=25, ax=ax)   

    shift = 0.5
    offset_outgoing_pos = {k:(x+shift,y+shift) for k,(x,y) in G.graph['pos'].items()}
    offset_incoming_pos = {k:(x-shift,y-shift) for k,(x,y) in G.graph['pos'].items()}

    #fig, ax = tapnx.plot_edge_labels(G, offset_outgoing_pos, ax=ax, font_size=20)
    #fig, ax = tapnx.plot_edge_labels(G, offset_incoming_pos, ax=ax, font_size=20)
    ax.axis('off')
    #print(filename)
    plt.show()
